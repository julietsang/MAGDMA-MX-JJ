...
pageextension 50125 "CfdiExportEXT" extends "CFDI Export Codes"
{
    layout
    {
        ...
        addlast(navigation)
        {
            group(ActionGroupCRM)
            {
                ...
                action(CRMSynchronizeNow)
                {
                    ...
                    trigger OnAction()
                    var
                        PaymentTerms: Record "CFDI Export Code";
                        CRMIntegrationManagement: Codeunit "CRM Integration Management";
                        PaymentCFDIEXPORTRecordRef: RecordRef;
                    begin
                        CurrPage.SetSelectionFilter(PaymentTerms);
                        PaymentCFDIEXPORTRecordRef.GetTable(PaymentTerms);
                        CRMIntegrationManagement.UpdateMultipleNow(PaymentCFDIEXPORTRecordRef, true);
                    end;
                }
                group(Coupling)
                {
                    ...
                    action(ManageCRMCoupling)
                    {
                        ...
                        Image = LinkAccount;
                        ToolTip = 'Create or modify the coupling to a Dataverse CFDI Export Code.';
                        ...
                        begin
                            ...
                            CRMOptionMapping.SetRange("Table ID", Database::"CFDI Export Code");
                            if CRMOptionMapping.Find('-') then
                                repeat


                                    Rec."Coupled to CRM" := true;
                                    if Rec.Modify(true) then Message('done');
